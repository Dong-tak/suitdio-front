...
export default function WidgetShell({
  ...
  useEffect(() => {
    if (isArrowMode && linkWidgets.length === 2) {
      const isAlreadyCreated = arrows.some(
        (arrow) =>
          (arrow.fromId === linkWidgets[0].id &&
            arrow.toId === linkWidgets[1].id) ||
          (arrow.fromId === linkWidgets[1].id &&
            arrow.toId === linkWidgets[0].id)
      );
  ...
  const addArrowWidget = () => {
    if (linkWidgets.length === 2) {
      ...
      // 새로운 화살표 객체 생성 (순수 getTsid() ID 사용)
      const newId = getTsid().toString();
      console.log("생성된 화살표 ID:", newId);
      ...
      console.log("새로 생성된 화살표:", {
        id: newArrow.id,
        fromId: newArrow.fromId,
        toId: newArrow.toId,
      });
